...
model_dir = '/data/models/Baichuan2-13B-Chat'
batch_size = 18
num_tokens = 256
...
# 生成文件名
model_name = model_dir.split('/')[-1]  # 从模型目录提取模型名称
output_file = f'.//{model_name}_{batch_size}batch.csv'

# 保存数据到文件
with open(output_file, 'w') as f:
    f.write('Model Name,Batch Size,Generate Token FPS,Prefill Latency,Single Batch Prefill Latency,Decode Latency,Single Batch Decode Latency,End-to-End Time,Num Tokens\n')
    f.write(f'{model_name},{batch_size},{fps:.3f},{prefill_time:.3f},{prefill_time/batch_size:.3f},{decode_time:.3f},{decode_time/batch_size:.3f},{consume_time:.3f},{num_tokens}\n')
